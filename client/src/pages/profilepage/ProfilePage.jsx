...
export default function ProfilePage(props) {
    ...
    const fetchInfo = async () => {
        await axios.get(`${BASE_URL}/api/user/info/${params.id}`, {
            headers:config
        })
        .then(res=>{
            setData(res.data)
                ...
                <Row className="mt-4">
                    ...
                    <Col sm={12} md={6}  className="d-flex align-items-center justify-content-center">
                        <div>
                            ...
                            <a className="error-cta fw-bold" href="/">Go back <CgArrowRight fontSize={36}/></a>
                        </div>
                    </Col>
                </Row>
                : 
                null}
                {data.length > 0? <Row className="d-flex flex-column pt-4">
                    <Col className="pt-4 d-flex flex-column">
                        <div className="d-flex flex-column align-items-center">
                            ...
                            <div className="text-center">
                                <h2 className="fw-bold mb-0">{data[0].firstname +" "+ data[0].lastname}</h2>
                                <p className="mb-0 text-muted">Gooly Member</p>
                            </div>
                        </div>
                        <div>
                            <p className="mb-2 mt-4 text-muted text-center small">Joined in {formatDate(data[0].date_created)}</p>
                        </div>
                    </Col>
                    <Col className="mt-4 d-flex justify-content-center">
                        <div style={{width: "100%", maxWidth: "400px"}}>
                            ...
                            <div className="mt-2 mb-2  d-flex justify-content-between align-items-center">
                                ...
                                <span className="text-muted">{formatDate(data[0].date_created)}</span>
                            </div>
                        </div>
                    </Col>
                </Row> : null}
